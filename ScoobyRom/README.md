...
In generall, different car models are equipped with different ECUs, therefore success varies.
...
This application is not a **ROM editor** (yet), you cannot change table values!
Remember, in this version the ROM file is only being read.
All additional data is saved into an extra XML file.
However, ScoobyRom has a RomRaider ECU def export feature, see chapter "RomRaider ECU def export XML".
...
No write access is needed there other than in case you need to edit configuration file `ScoobyRom.config` - see chapter 5.1.
...
ScoobyRom is compiled as 32 bit application due to dependency *Gtk#* being limited to 32 bit. (There is no official 64 bit version so far. Underlying native *Gtk+* toolkit is available as 64 bit nowerdays, though.)
...
* Advanced: Clone source repository, assuming [git](https://www.git-scm.com/) is installed:
...
##### Compilation
For instructions and hints see file [DEVELOPMENT.txt](DEVELOPMENT.txt).
...
*	Windows 8.1
	...
	* Tested. Windows 8.1 ships with .NET v4.0 pre-installed. Upgrading to newer version does not hurt (v4.5 runs 4.0 apps and might be slightly faster).

* Older Windows versions:

	* You might already got .NET 4 or later version through *Windows Update*.
	...
	* Links
		...
		* [Microsoft .NET homepage](https://www.microsoft.com/net)
...
Note: Some software like *MonoDevelop* depends on Gtk#, too.
...
*	**Mono for Windows** is not needed if you have a sufficient Microsoft .NET runtime on your system already!
...
Use any text editor to view/edit the file.
Recommendation for Windows: **Notepad++** (free OSS, lots of features) 	<http://notepad-plus.sourceforge.net/uk/site.htm>
...
* Cons:
  ...
  WARNING: In case of existing file, ScoobyRom does not ask for permission,
    it immediatly overwrites existing file! Be careful not to overwrite XML files produced by any other software!!!
...
Somewhat larger range will be very fast already, might work for similar ROMs:

	<tableSearch start="0x80000" end="0x95000" />


Note: Searching through unsuitable range (too large or whole file) might introduce false results (tables)!
...
gnuplot windows (interactive terminal) might provide "Save As" functionality.
...
Although gnuplot supports parsing text data via standard input, I prefer binary transfer where full accuracy is guaranteed (32 bit floating point), similar like a control unit would see the maps. No need to generate text and parse in gnuplot, no worries about decimal places etc.
...
must live in app EXE directory. Errors can result in empty plots.
...
Feel free to modify those - change colors, layout, default view etc. Simpy replot to review changes.

A few (text labeling) commands are generated within the app on the fly, you need to change app source code for this.
...
*	Visualization (Ctrl+Space): Either displays colored table values (3D) or 2D values + line graph in bottom tab pane.
